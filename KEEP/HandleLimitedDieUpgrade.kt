...
class HandleLimitedDieUpgrade(
    ...
    operator fun invoke(player: Player, only: List<DieSides>, discardAfterUse: Boolean): Die? {
        ...
        val highestSidedDice = dice.dice
            .filter { die -> only.any { allowed -> allowed.value == die.sides } }
            .groupBy { it.sides }
            .maxByOrNull { it.key }?.value ?: return null

        // Among the highest-sided dice, select the one with the lowest value
        val dieToUpgrade = highestSidedDice.minByOrNull { it.value } ?: return null
