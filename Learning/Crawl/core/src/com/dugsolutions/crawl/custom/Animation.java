package com.dugsolutions.crawl.custom;
/*******************************************************************************
...
/**
 * * An Animation stores a list of {@link TextureRegion}s representing an animated sequence, e.g. for running or
 * jumping. Each
...
public class Animation
{
	 ...
	 */
	public Animation(float frameDuration, Array<TextureRegion> keyFrames, int playType)
	{
		this.frameDuration = frameDuration;
		this.animationDuration = keyFrames.size * frameDuration;
		this.keyFrames = new TextureRegion[keyFrames.size];
	 ...
	 * spent in the
	 * state this Animation instance represents, e.g. running, jumping and so on. The mode specifies whether the
	 * animation is
	 * looping or not.
	 *
	 * @param stateTime
	...
	public int getKeyFrameIndex(float stateTime)
	{
		...
		switch (playMode)
		{
			...
			case LOOP_PINGPONG:
				...
				frameNumber = keyFrames.length - 1 - Math.abs(frameNumber - keyFrames.length + 1);// keyFrames.length -
																									// 2 - (frameNumber
																									// -
																									// keyFrames.length);
				break;
