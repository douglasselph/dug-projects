...
class CombinationGeneratorTest {
    ...
    fun invoke_withSingleDie_generatesOneCombination() {
        ...
        val combination = result.choices[0]
    ...
    fun invoke_withTwoDice_generatesThreeCombinations() {
        ...
        val hasDie1 = result.choices.any { it.values.dice.size == 1 && it.values.dice[0] == testDieValue1 }
        val hasDie2 = result.choices.any { it.values.dice.size == 1 && it.values.dice[0] == testDieValue2 }
        val hasBoth = result.choices.any { it.values.dice.size == 2 && it.values.dice.contains(testDieValue1) && it.values.dice.contains(testDieValue2) }
    ...
    fun invoke_withThreeDice_generatesSevenCombinations() {
        ...
        // Assert
        assertEquals(7, result.choices.size)

        // Check all possible combinations
        val hasDie1 = result.choices.any { it.values.dice.size == 1 && it.values.dice[0] == testDieValue1 }
        val hasDie2 = result.choices.any { it.values.dice.size == 1 && it.values.dice[0] == testDieValue2 }
        val hasDie3 = result.choices.any { it.values.dice.size == 1 && it.values.dice[0] == testDieValue3 }
        val hasDie1And2 = result.choices.any { it.values.dice.size == 2 && it.values.dice.contains(testDieValue1) && it.values.dice.contains(testDieValue2) }
        val hasDie1And3 = result.choices.any { it.values.dice.size == 2 && it.values.dice.contains(testDieValue1) && it.values.dice.contains(testDieValue3) }
        val hasDie2And3 = result.choices.any { it.values.dice.size == 2 && it.values.dice.contains(testDieValue2) && it.values.dice.contains(testDieValue3) }
        val hasAll = result.choices.any { it.values.dice.size == 3 && it.values.dice.contains(testDieValue1) && it.values.dice.contains(testDieValue2) && it.values.dice.contains(testDieValue3) }
    ...
    fun invoke_withPipModifier_extendsAllCombinations() {
        ...
        assertEquals(6, result.choices.size)

        // Check base combinations
        val baseCombinations = result.choices.filter { it.addToTotal == 0 }
        assertEquals(3, baseCombinations.size)

        // Check extended combinations
        val extendedCombinations = result.choices.filter { it.addToTotal == pipModifier }
    ...
    fun invoke_withZeroPipModifier_doesNotExtendCombinations() {
        ...
        assertEquals(3, result.choices.size)
        assertTrue(result.choices.all { it.addToTotal == 0 })
    }
}
