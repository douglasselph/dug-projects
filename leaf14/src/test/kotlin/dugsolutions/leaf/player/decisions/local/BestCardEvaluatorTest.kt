...
class BestCardEvaluatorTest {
    ...
    fun setup() {
        ...
        flowerCard2 = FakeCards.weakFlowerCard.copy(id = 8)
    }
    ...
    fun invoke_whenSameEvaluationDifferentCounts_returnsLeastOwned() = runBlocking {
        // Arrange
        val cards = listOf(rootCard1, rootCard2)
        every { mockPlayer.allCardsInDeck } returns listOf(rootCard1.id, rootCard1.id) // rootCard1 appears twice
    ...
    fun invoke_whenSameEvaluationAndCount_returnsByFlourishTypePriority() = runBlocking {
        // Arrange
        val cards = listOf(rootCard1, vineCard1, flowerCard1)
