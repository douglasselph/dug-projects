...
public class MatrixTrackingGL implements GL, GL10, GL10Ext, GL11, GL11Ext {
    ...
    GL11Ext mgl11Ext;
    int mMatrixMode;
    MatrixStack mCurrent;
    MatrixStack mModelView;
    MatrixStack mTexture;
    MatrixStack mProjection;

    final static boolean _check = false;
    ...
    public Matrix4f getMatrix() {
    	...
    	getMatrix(values, 0);
    	return new Matrix4f().setValues(values);
    }
    ...
    private void check() {
        ...
        for(int i = 0; i < 16; i++) {
            if (mCheckA[i] != mCheckB[i]) {
                Log.d("GLMatWrap", "[" + i + "] " + mCheckA[i] + " != " + mCheckB[i]);
                fail = true;
            }
        }
        if (fail) {
            throw new IllegalArgumentException("Matrix math difference (" + which + ")");
        }
    }
}
