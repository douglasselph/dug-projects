...
public class Camera {
	...
	public interface OnPositionChanged {
		void positionChanged(Camera cam);
	};
	...
	protected Vector3f			mUnitOut;								// computed
	protected Vector3f			mUnitLeft;								// computed
	...
	/** If true, then applyLookAt should be called */
	protected boolean			mNeedsLookAt;
	/** If true, then applyFrustrum should be called */
	protected boolean			mNeedsFrustrum;
	/** When the look at position changes */
	protected OnPositionChanged	mOnPositionChanged;
	...
	/**
	 * Called to update the camera position. Called frequently.
	 * 
	 * @param gl
	 */
	public synchronized void applyLookAt(MatrixTrackingGL gl) {
		...
		GLU.gluLookAt(gl, mCameraPos.getX(), mCameraPos.getY(),
				mCameraPos.getZ(), mLookAtPos.getX(), mLookAtPos.getY(),
				mLookAtPos.getZ(), mUp.getX(), mUp.getY(), mUp.getZ());
		// Perhaps another way to rotate the camera around a point:
		// gl.glRotatef(angle, x, y, z);

		if (mOnPositionChanged != null)
		{
			mOnPositionChanged.positionChanged(this);
		}
	}

	/**
	 * Call applyFrustrum() if relevant data has changed.
	 * 
	 * @param gl
	 */
	public void checkUpdateFrustrum(MatrixTrackingGL gl) {
		if (mNeedsFrustrum)
		{
			mNeedsFrustrum = false;
			applyFrustrum(gl);
		}
	...
	/**
	 * Call applyLookAt() if relevant data has changed.
	 */
	public void checkUpdateLookAt(MatrixTrackingGL gl) {
		if (mNeedsLookAt)
		{
			mNeedsLookAt = false;
			applyLookAt(gl);
	...
	protected Vector3f getUnitLeft() {
		if (mUnitLeft == null)
		{
			...
			if (mUnitLeft.equals(Vector3f.ZERO))
			{
				if (mUnitOut.getX() != 0)
				{
					mUnitLeft.set(new Vector3f(mUnitOut.getY(), -mUnitOut
							.getX(), 0));
				} else
				{
					mUnitLeft.set(new Vector3f(0, mUnitOut.getZ(), -mUnitOut
							.getY()));
				}
			}
		}
	 ...
	 * @param zFar
	 */
	void gluPerspective(MatrixTrackingGL gl, float fovy, float aspect,
	...
	//
	// return new Vector4f(winCoords[0], winCoords[1], winCoords[2],
	// clipVec[3]);
	// }
	//
	...
	// public Vector3f getWorldPosition(MatrixTrackingGL gl, float px, float py,
	// float pz) {
	// gl.glMatrixMode(GL10.GL_MODELVIEW);
	...
	protected synchronized void rotate(float ax, float ay) {
		if (ax != 0 && ay != 0)
		{
			...
			float curOutDist = getDistFromTarget();
			mLookAtPos = new Vector3f(mUnitOut).multiply(curOutDist).add(
					mCameraPos);
			mNeedsLookAt = true;
		}
	}
