...
For Milestone-2 Human Baseline review, start with [Human Baseline Specification](doc/HUMAN_BASELINE.md). For balance research workflow, start with [Using the Simulator](doc/USING_THE_SIMULATOR.md). If you instead need to understand how the engine is wired together, start with [Code Architecture](doc/CODE_ARCHITECTURE.md). Critter Reward, Wound Resolution, Graft Placement, Buy, Cultivation, Battle, and Butterfly Result are now certified Human Baseline areas. Their accepted behavior contracts, focused verification guidance, and current Milestone-2 progress are recorded in the Human Baseline specification.
