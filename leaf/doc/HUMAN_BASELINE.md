...
1. **Keep buying while another legal purchase remains.** `BuyCoordinator` loops and regenerates legal options after every committed purchase.
2. **Choose Plant-vs-die from current development, not round number.** Each grafted Plant counts as 15 development points; dice development is total owned die sides. Equal development is 50/50 and a 3-point imbalance is about 85/15 toward the weaker side. If the Creature has fewer than 2 Plants, the state-based low-Plant safety rule takes precedence at 90% Plant / 10% die.
3. **Plan short multi-buys, not just the next expensive item.** Human Baseline tries meaningful groups of remaining Hand dice and looks ahead at **no more than two purchases at a time**. It assigns purchases to those groups, projects the second purchase when present, and chooses the plan with the lowest overpay concern. Thus `5 + 5 + 5` may be compared as one buy (`15`) versus two buys (`5 + 10`), but never as a three-buy `5 + 5 + 5` plan. This is what allows `R5 for 5; D10 for 10` to beat `V11 paid with 15`.
4. **Overpay concern is nonlinear.** The default plan penalty for overpay 0/1/2/3/4 is 0/1/3/7/15. One point is minor; repeated or large overpay becomes rapidly unattractive.
5. **Die purchases still prefer the largest die their assigned group can buy.** If two complete plans are otherwise equally efficient and the current category is Die, the plan with the larger first die wins. Returned D4s remain real Buy options.
6. **Plant purchases retain an exponential expensive-tier tendency.** With all normal 5/7/9/11/14/17 tiers available, the raw weights remain roughly 2% / 4% / 7% / 13% / 25% / 48%. The two-purchase overpay planner can nevertheless make a cheaper Plant the sensible first purchase when it produces a cleaner multi-buy sequence.
7. **Project future graft legality and supply.** The look-ahead sees all still-stocked Grove items, including Plants not legal *yet*. A projected Vine can therefore make a Flower legal later in the same Buy phase. Real coordinator legality is rechecked before every cached step.
8. **Normally preserve 2 Bees and 1 Worm, but the Bee reserve is soft during Buy.** A Bee may be spent even when the player owns only one or two if it upgrades an otherwise-limited purchase by one useful tier.
9. **Bee tier-step probability depends on the purchase and Bee count.** With one Bee, die bridges default to about D4→D6 4%, D6→D8 11%, D8→D10 26%, D10→D12 50%. Each additional Bee doubles the odds; with 3 Bees those become about 14%, 32%, 58%, and 80%. D12→D20 is never a Bee bridge.
10. **Plant Bee bridges use a flat starting willingness.** An eligible Plant bridge starts at 20% with one Bee, about 33% with two, and 50% with three. A Bee may bridge dice-group budgets 5/6→7, 7/8→9, 9/10→11, 12/13→14, and 15/16→17 when that next legal tier exists. Budgets 11, 14, and 17+ have no +2 bridge to another standard tier.
11. **Worm bridges are rarer and only one point wide.** At least two Worms are required. Default willingness is about 5%/10%/17%/30% with 2/3/4/5 Worms, and the desired purchase must be exactly one point above the dice-group budget.
12. **Critter willingness is shared across candidate groupings.** The planner samples one Bee willingness roll and one Worm willingness roll before evaluating candidate groupings. Every grouping sees the same roll; candidate branches do not get independent lottery tickets.
13. **Use card-specific scoring only inside a chosen Plant cost tier.** Duplicate Plants receive no generic diversity penalty.
14. **Use strategy RNG only for Human Baseline probabilities.** Category balance, Plant-tier weighting, shared Critter willingness, fallback Critter spending, and Bee-vs-Worm variation never consume mechanical RNG.

The cross-cutting tuning values are centralized in `HumanBaselinePolicy`, including Plant-equivalent power, Plant-tier exponential weights, Bee tier-step odds, and overpay penalties. See [`HUMAN_BASELINE_POLICY.md`](HUMAN_BASELINE_POLICY.md) for the tuning/override model.

#### Buy behavior-contract tests

`HumanBaselineBuyStrategyTest` contains a clearly labeled **Human Baseline Behavior Contract** section. Its tests directly demonstrate category balance, exponential Plant-tier selection, two-purchase look-ahead, exact planned payment groups, future Flower legality after a projected Vine, the reversed Bee/die curve, higher Plant bridge tiers, conservative Worm bridges, one shared Critter willingness roll, Critter reserves, and equal-overpay Bee/Worm fallback behavior. `BuyCoordinatorTest` additionally proves actual repeated Human Baseline purchases and Grove supply regeneration between purchases.
