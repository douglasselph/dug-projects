...
B15B1a is being implemented in timeout-safe micro-slices rather than as one ten-effect patch. Two slices are complete:
...
- [ ] remaining B15B1a deterministic transforms — `DOUBLE_ONE_DIE`; `FLIP_OWN_DIE_TO_OPPOSITE_FACE`; `SET_DIE_SHOWING_2_PLUS_TO_1_AND_GAIN_VP_PER_ONE`; `SET_DIE_UP_TO_D12_TO_MAX`; `SET_LOWEST_VALUE_DIE_TO_MAX`.

These completed slices change only downstream Battle target alignment for straightforward deterministic raises. They do not complete B15B1a, do not change Cultivation targeting, and do not certify Effect Choices.

### Remaining B15 checkpoints

- [ ] **B15B1a** — pure deterministic own-die transforms (fixed +1/+4 and straightforward +3/creature-count raise slices complete; remaining transforms still pending)
