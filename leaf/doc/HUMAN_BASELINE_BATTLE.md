...
B15B2 also aligns source-die decisions that matter immediately in Battle. Discard-before-Draw effects use expected visible Draw information without pre-rolling or pre-committing later decisions. For Forget-Me-Not, the Discard source determines the die size that will be rerolled and immediately placed, so source selection uses that die size's mathematical expected roll and the best currently legal expected placement. Its current Discard face is intentionally irrelevant because the executor rerolls it. The Mulch-from-Hand/Discard source decisions are intentionally unchanged: those effects create `PENDING_MULCH`, which is not normalized into usable Mulch until Battle Cleanup, so there is no distinct immediate Battle placement/support realization to optimize when choosing the source.

B15B3 now begins the random own-die target audit. For Root on a Roll (`REROLL_DIE_UNTIL_3_PLUS_IGNORE_ROLL_REWARDS`) and Root Recall's Battle one-die branch (`DISCARD_ANY_NUMBER_OF_DICE_AND_REDRAW_OR_REROLL_ONE_IN_BATTLE`), downstream target selection uses expected affected-row Battle analysis rather than raw expected face gain. Root on a Roll uses the exact repeated-reroll-until-3+ accepted-face expectation; Root Recall uses an ordinary fair-reroll expectation. No hypothetical mechanical RNG is consumed, the live die stays in its existing row, and exact tactical ties remain strategy-randomized. Top-level enabled-Plant analysis uses the same Root-on-a-Roll expectation, preserving action/target compatibility.

Root Awakening (`UPGRADE_DIE_AND_USE_NOW`) follows the same expectation-before-RNG contract. For each legal Battle target, Human Baseline projects the exact normal one-step replacement size and its fair expected roll on the target's current Strike row; the rules engine still owns normal-ladder and Graft Bed legality, and no replacement roll or placement is pre-resolved. Enabled-Plant analysis uses the same expected projection. Normal Compost (`UPGRADE_DIE_FROM_HAND`) is intentionally unchanged because its current concrete Round-card sources are Cultivation-only and the upgraded die goes to Discard rather than being rolled/used immediately. Overgrowth's two-step skip-missing policy remains isolated in the existing B13 evaluator.

Gust of Petals (`REROLL_ONE_DIE_AND_REROLL_HIGHER_OPPOSING_DICE_IN_STRIKE_ROW`) completes B15B3. The first own-die target is chosen before RNG, so Human Baseline uses mathematical reroll expectation and evaluates the resulting immediate Battle state together with the best currently visible expected later Strike-Row branch for the forced opposing rerolls. Opposing dice that are above the actor's projected lowest die in that candidate row contribute their own fair expected reroll changes, so a large local own-die gain can be rejected when its later collateral is tactically worse. The later Strike-Row choice remains a separate post-reroll decision and is not pre-committed here; B15D still owns alignment of that downstream row choice after the actual own reroll is known. No hypothetical mechanical RNG is consumed. `BattleEnabledPlantAnalyzer` reuses the same Gust target analyzer so top-level enabled-Plant value and downstream own-die target selection share the same expected realization.

B15B4 completes compound own-die alignment. Root Kindred (`SET_DIE_TO_MATCH_ANOTHER`) evaluates each legal source/target pair as one deterministic Battle realization: the source die is unchanged, the target die is set exactly to the source's current value, and the signed target-row change is passed through shared Battle analysis. This allows a smaller raw copied-value gain that flips a Strike to beat a much larger numeric gain on a row that still loses. `BattleEnabledPlantAnalyzer` enumerates the same legal source/target pair shape for enabled-Plant valuation, preserving compatible reasoning without storing a stale target commitment. Vine and Dine (`TRASH_CRITTER_TO_RAISE_DIE_PLUS_5`) likewise evaluates each legal Critter+die candidate using the chosen die's exact capped +5 Battle realization. The pre-existing Bee/Worm availability adjustment is retained only as a secondary combined-choice preference; it does not add future-Battle conservation or stop Human Baseline from spending a Critter to improve the current Battle. Non-Battle pair/Critter scoring remains unchanged, and exact tactical ties continue to use `StrategyRandomizer`.

B15C aligns Root Well's Battle branch/target decision. `GAIN_WATER_AND_SPEND_1_TO_REROLL_TWO_OWN_OR_ONE_OPPONENT_BATTLE_DIE` now compares each legal complete branch by honest pre-RNG expectation through shared Battle analysis: both committed own-die rerolls are aggregated on their actual rows, while the opponent branch projects the selected opponent die's expected reroll on its current row. The strategy does not roll hypothetically or defer/replace the legally committed targets after randomness.

B15D begins with the two own-die swap-pair decisions. For the optional `DISCARD_ONE_DIE_DRAW_ONE_AND_SWAP_TWO_OWN_DICE_IN_BATTLE` swap, Human Baseline now projects both affected Strike Rows after exchanging the selected live dice and can decline the optional swap when the complete Battle result is not worthwhile. For `DRAW_ONE_DIE_AND_SWAP_TWO_OWN_DICE_RAISE_ONE_PLUS_2_IN_BATTLE`, the same projection also applies the printed +2 to the designated source die after it has moved to the other row, so source/target direction is part of the realization. Both decisions use shared multi-row Battle analysis rather than independent face-value or RowNeed arithmetic. This slice does not alter any Strike-Row target decision.

B15D-2 aligns the two deterministic Strike-Row branches. Root & Scoot (`RAISE_DIE_PLUS_1_AND_WITHDRAW_FROM_STRIKE_SQUARE`) still chooses its +1 die first, then makes a fresh mandatory withdrawal-row decision. That row choice now projects the actor leaving the Strike entirely: current Strike VP/winning position can be lost, close contests are worth preserving, and leaving a row in which the actor is currently exposed to a Wound can be tactically useful because withdrawn players no longer participate in Strike resolution. Vine & Punishment (`SET_ANY_DIE_TO_3_OR_REDUCE_OPPOSING_STRIKE_ROW_BY_3`) now projects the exact capped -3 reduction to every participating opposing die in each legal row and compares the resulting real multiplayer Strike state. Both row choices therefore use shared deterministic Battle analysis rather than first-row/enum ordering or generic RowNeed.

B15D-3 completes Strike-Row alignment for Gust of Petals. The card's first own-die reroll has already happened before `chooseStrikeRow(...)` is asked, so the fresh DecisionContext contains that actual result. Human Baseline now evaluates each legal row using the actor's actual current lowest die in that row to determine exactly which opposing dice would be forced to reroll. Those still-unknown opposing rerolls are projected by fair expectation and the complete expected row result is compared through shared Battle analysis. Thus the downstream row choice uses actual-after-RNG information for the first reroll, expectation-before-RNG for the later forced rerolls, consumes no mechanical RNG, and no longer falls back to first-row/`RowNeed` bias.


B15E1 aligns Plant/opponent-Plant decisions without adding deep future-Creature search. For the three Battle `chooseOpponentPlantWound(...)` paths, target scoring now distinguishes immediate Plant capability from generic card-loss value. A face-up opponent Plant that has a strong realizable current-Battle Main is a better Flip/Wound target; Shift Happens also recognizes that flipping a spent opponent Root/Vine face up can *help* that opponent and therefore avoids doing so when a less harmful legal target exists. Snip choices preserve the existing card-loss/removal value because an already-spent Plant has no immediate activation to deny. The projection uses only public opponent board/Battle information and does not reconstruct hidden Wisp identities. `REUSE_SPENT_ROOT_OR_VINE_EFFECT` / Vine and Again now chooses among its executable spent Root/Vine choices with the existing one-step `BattleEnabledPlantAnalyzer`, so a reusable Plant whose immediate effect changes the current Strike can beat the first legal or merely higher generic card-score choice. The reused Plant remains spent and no recursive/deep combo search is added.

B15E2 aligns the bounded resource/player/Wisp-set seams. Bee-loved Bloom keeps its actor-side gain source-invariant, but `BattleBeeSourceAnalyzer` distinguishes opponent sources by the immediate public Bee Support danger that stealing that Bee denies in the current Battle; Done opponents contribute no such denial value. This remains one-step visible-information reasoning and does not infer hidden Wisp identities or model future rounds. The other resource/player/Wisp-set choices retain simple bounded heuristics where no stronger immediate target-specific Battle realization is required. This work does not certify the broader Effect Choices area.
...
## B15 closeout — action / target / branch alignment COMPLETE

B15 is complete. Battle is now **CERTIFIED** after the later B16/B17 verification work and Stage C regression/certification gates. The B15A inventory identified 50 current Battle effect/mechanism pairings; all 50 now have a documented disposition in `HUMAN_BASELINE_BATTLE_PLAN.md`. B15 closes top-level action versus downstream target/branch mismatches using bounded current-Battle reasoning, while leaving the broader Effect Choices major area uncertified.

Completed B15 slices are B15B1a, B15B1b, B15B2, B15B3, B15B4, B15C, B15D, B15E1, B15E2, B15E3-1, B15E3-2, B15F1, B15F2-1, B15F2-2, B15F2-3, and B15G representative integration. B15E3 and B15F2 are therefore complete aggregates.

The final random-information contract is: commit targets that the rules require before RNG; reason by honest mathematical expectation while results are unknown; never consume mechanical RNG for hypothetical analysis; after RNG, use the actual result and a fresh decision context for any later choice the rules ask separately; and never pre-commit a later placement/row merely to value an earlier random action. The B15F1 audit/repairs found the audited B15 seams consistent with that contract, including Wispquake, rerolls, upgrade/use-now, Draw/Forget-Me-Not, Gust of Petals, and Petal To Die 4.

The B13-special end-to-end contract is also aligned. Overgrowth top-level willingness follows the dedicated two-step upgrade policy's largest-result family and expected-current-Battle tie-break. Pollen Theft top-level willingness uses the same complete multi-row realization and meaningful-VP gate as `BattlePollenTheftEvaluator`. Wisp's Resolve requires a row that passes `BattleImmediateStrikeResolveEvaluator`'s established immediate-resolution policy, including the current lead/support-capacity/Done semantics, rather than receiving generic value for any current lead.
...
B17 focused Battle verification and the Stage C regression/certification gates are complete. Battle is **CERTIFIED**.

### B15 closeout verification note

The B15G-2 focused unit and representative integration verification commands were run in the portable offline Gradle environment, but each exceeded the execution window during Gradle compilation/setup before any test reached execution. Exact observed result for that earlier attempt: **0 tests executed, 0 test failures reported, and no green result was claimed**. That historical timeout did not certify Battle; the later B17 work and successful C2 full regression supplied the required verification before C3 certification.


## Certification

**Battle is CERTIFIED.** The approved Battle behavior contract and implementation checkpoints B1–B17 are complete. C1 durable documentation is complete, and C2 completed the full `test integrationTest simulationCheck` regression successfully before this C3 certification. The Battle strategy API remains three hooks, so overall Human Baseline progress is **3 of 8 major areas certified; 6 of 30 strategy hooks certified**.
