...
B15 target alignment is intentionally being added in bounded slices. For the straightforward deterministic own-die transforms `RAISE_ANY_DIE_PLUS_1`, `RAISE_DIE_PLUS_3`, `RAISE_DIE_PLUS_4`, `RAISE_DIE_PLUS_1_PER_GRAFTED_VINE_OR_FLOWER`, `RAISE_DIE_PLUS_1_PER_ROOT_OR_VINE`, `DOUBLE_ONE_DIE`, and `FLIP_OWN_DIE_TO_OPPOSITE_FACE`, downstream Battle target selection now projects each legal die's exact deterministic value change through the same shared own-row Battle analysis used by top-level Battle valuation. The creature-count raises use the current Creature composition to determine their real raise amount; Double uses its capped doubled face value; opposite-face Flip uses the actual signed face change. Raw numeric value change is therefore subordinate to the actual immediate Strike consequence. Cultivation targeting remains unchanged.
