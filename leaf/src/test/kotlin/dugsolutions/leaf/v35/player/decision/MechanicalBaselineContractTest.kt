 ...
 */
class MechanicalBaselineContractTest {

    @Test
    fun specification_coversEveryDecisionArea() {
        assertEquals("Mechanical Baseline", MechanicalBaseline.NAME)
        assertEquals(0, MechanicalBaseline.STRATEGY_LEVEL)
        assertEquals(DecisionArea.entries.toSet(), MechanicalBaseline.rules.keys)
        assertTrue(MechanicalBaseline.rules.values.all { it.isNotBlank() })
    }

    @Test
    fun createsFreshIndependentDirectors() {
        val first = DecisionDirector.mechanicalBaseline()
        val second = DecisionDirector.mechanicalBaseline()

        assertNotSame(first, second)
        assertNotSame(first.buy, second.buy)
        assertNotSame(first.effect, second.effect)
        assertEquals(first::class, DecisionDirector.baseline()::class)
    }

    @Test
    fun cultivation_prefersDrawThenStopsInsteadOfOptionalSupport() {
        val strategy = DecisionDirector.mechanicalBaseline().cultivation
    ...
    fun battle_prefersDrawAndFinishesBeforeOptionalSupport() {
        val strategy = DecisionDirector.mechanicalBaseline().battle
    ...
    fun buy_choosesFirstItemAndMinimumSufficientPayment() {
        val strategy = DecisionDirector.mechanicalBaseline().buy
    ...
    fun placementAndWound_chooseFirstLegalChoice() {
        val director = DecisionDirector.mechanicalBaseline()
    ...
    fun critterReward_choosesFewerOwnedTypeAndBeeOnTie() {
        val strategy = DecisionDirector.mechanicalBaseline().reward
    ...
    fun butterfly_keepsHigherValueAndOriginalOnTie() {
        val strategy = DecisionDirector.mechanicalBaseline().support
    ...
    fun effectTargeting_choosesFirstLegalTarget() {
        val strategy = DecisionDirector.mechanicalBaseline().effect
        val choices = listOf(
