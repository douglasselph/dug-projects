...
class DoubleOneDieTest {

    @Test
    fun invoke_withHandScope_doublesTargetHandDie() {
        ...
        assertEquals("Doubled one die in player 1's hand", entry.detail)
        assertEquals(listOf(8 to 6), entry.dice.map { it.sides to it.value })
    }

    @Test
    fun invoke_withBattleScope_doublesTargetBattleGridDie() {
        val chronicle = GameChronicle()
        val card = loadCard()
        val targetDie = TestDie(8, 4)
        ...
        val entry = assertIs<GameEntry.GameCardEffect>(chronicle.getEntries().single())
        assertEquals(card.effect, entry.effect)
        assertEquals("Doubled one die in player 1's STRIKE_1 battle square", entry.detail)
    ...
    private fun loadCard(): GameCard {
        return GameCardRegistry()
            ...
            .getAllCards()
            .first()
            .copy(effect = CardEffect.DOUBLE_ONE_DIE)
    }
