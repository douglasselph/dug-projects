...
class GameCardIDsTest {
    ...
    private lateinit var loadedCards: List<GameCard>
    private val randomizer: Randomizer = Randomizer.create(42) // Fixed seed for reproducibility
    
    @BeforeEach
    fun setup() {
        val registry = GameCardRegistry()
        registry.loadFromCsv(Commons.CARD_LIST)
        loadedCards = registry.getAllCards()
        cardManager = GameCardManager(GameCardsFactory())
        cardManager.loadCards(loadedCards)
    }
    
    private fun createTestCardId(index: Int): GameCardID = GenGameCardID.generateId("test_card_$index")
    ...
    fun map_whenAllCardsExist_returnsTransformedList() {
        // Arrange
        val card1 = loadedCards[0]
        val card2 = loadedCards[1]
        val cards = GameCardIDs(cardManager, listOf(card1.id, card2.id), randomizer)
        
        // Act
        val result = cards.map { it.name }
        
        // Assert
        assertEquals(listOf(card1.name, card2.name), result)
    }

    @Test
    fun map_whenSomeCardsDontExist_skipsMissingCards() {
        // Arrange
        val card = loadedCards[0]
        val missingCardId = createTestCardId(1)
        val cards = GameCardIDs(cardManager, listOf(card.id, missingCardId), randomizer)
        
        // Act
        val result = cards.map { it.name }
        
        // Assert
        assertEquals(listOf(card.name), result)
    }
    ...
    fun getCard_whenIndexInRange_returnsCorrectCard() {
        // Arrange
        val expectedCard = loadedCards[1]
        val cards = GameCardIDs(
            cardManager, listOf(
                loadedCards[0].id,
                expectedCard.id,
    ...
    fun getCard_whenSingleItemAndIndexZero_returnsCorrectCard() {
        // Arrange
        val expectedCard = loadedCards[0]
        val cards = GameCardIDs(cardManager, listOf(expectedCard.id), randomizer)
    ...
    fun getCard_whenAccessingLastValidIndex_returnsCorrectCard() {
        // Arrange
        val expectedCard = loadedCards[2]
        val cards = GameCardIDs(
            cardManager, listOf(
                loadedCards[0].id,
                loadedCards[1].id,
    ...
    @Test
    fun concurrentAccess_whenMultipleThreadsReadAndWrite_returnsConsistentState() {
        // Arrange
        val gameCardIDs = GameCardIDs(cardManager, emptyList(), randomizer)
        val iterations = 1000
        val threadCount = 10
        val addedCards = mutableSetOf<GameCardID>()
        val cardIdToThreadAndIteration = mutableMapOf<GameCardID, Pair<Int, Int>>()
        ...
        val threads = List(threadCount) { threadId ->
            Thread {
                repeat(iterations) { iteration ->
                    ...
                    // Track which thread and iteration created this cardId
                    synchronized(cardIdToThreadAndIteration) {
                        if (cardIdToThreadAndIteration.containsKey(cardId)) {
                            val (existingThread, existingIteration) = cardIdToThreadAndIteration[cardId]!!
                            println("Duplicate cardId found: $cardId")
                            println("  First created by thread $existingThread at iteration $existingIteration")
                            println("  Second created by thread $threadId at iteration $iteration")
                        }
                        cardIdToThreadAndIteration[cardId] = Pair(threadId, iteration)
                    }
                    
                    gameCardIDs.add(cardId)
                    addedCards.add(cardId)
                    
                    // Read the state
                    val currentCards = gameCardIDs.cardIds
                    assertTrue(currentCards.contains(cardId)) // Should see our own addition
                    
                    // Verify duplicates
                    assertEquals(currentCards.size, currentCards.toSet().size)
                }
            }
        }
        ...
        // Assert
        val finalCards = gameCardIDs.cardIds
        assertTrue(finalCards.containsAll(addedCards)) // Should contain all added cards
        
        // Detailed duplicate analysis
        val duplicates = finalCards.groupBy { it }
            .filter { it.value.size > 1 }
            .mapValues { it.value.size }
        
        if (duplicates.isNotEmpty()) {
            println("\nDuplicate Analysis:")
            println("Total cards in final state: ${finalCards.size}")
            println("Unique cards in final state: ${finalCards.toSet().size}")
            println("Number of duplicate cardIds: ${duplicates.size}")
            println("\nDuplicate Details:")
            duplicates.forEach { (cardId, count) ->
                val (threadId, iteration) = cardIdToThreadAndIteration[cardId]!!
                println("CardId: $cardId")
                println("  Created by thread $threadId at iteration $iteration")
                println("  Appears $count times in final state")
            }
        }
        
        assertEquals(finalCards.size, finalCards.toSet().size, "Found ${duplicates.size} duplicate cardIds")
    }
} 
