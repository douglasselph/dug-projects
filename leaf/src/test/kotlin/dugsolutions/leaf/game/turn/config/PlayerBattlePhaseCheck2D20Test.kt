...
class PlayerBattlePhaseCheck2D20Test {
    ...
    fun setup() {
        ...
        SUT = PlayerBattlePhaseCheck2D20(mockMarket, mockDieFactory)
    ...
    fun isReady_whenPlayerHasOneD20Die_returnsFalse() {
        ...
        every { mockMarket.hasCards } returns true
        every { mockMarket.hasDice } returns true
    ...
    fun isReady_whenPlayerHasNoD20DiceButMarketIsEmpty_returnsTrue() {
        ...
        every { mockMarket.hasCards } returns false
        every { mockMarket.hasDice } returns false
    ...
    fun giftTo_whenPlayerIsNotReadyAndMarketHasD20_givesD20ToPlayer() {
        ...
        every { mockMarket.hasCards } returns true
        every { mockMarket.hasDice } returns true
        every { mockMarket.hasDie(DieSides.D20.value) } returns true
        ...
        verify { mockMarket.removeDie(fakeD20) }
    ...
    fun giftTo_whenPlayerIsNotReadyButMarketHasNoD20_doesNothing() {
        ...
        every { mockMarket.hasCards } returns true
        every { mockMarket.hasDice } returns true
        every { mockMarket.hasDie(DieSides.D20.value) } returns false
        ...
        verify(exactly = 0) { mockMarket.removeDie(any()) }
    ...
    fun giftTo_whenPlayerIsReady_doesNothing() {
        ...
        // Assert
        verify(exactly = 0) { mockMarket.hasDie(any()) }
        verify(exactly = 0) { mockDieFactory(any<DieSides>()) }
        verify(exactly = 0) { mockMarket.removeDie(any()) }
