...
class PlayerBattlePhaseCheckBloomTest {
    ...
    fun setup() {
        ...
        SUT = PlayerBattlePhaseCheckBloom(mockMarket)
    }
    ...
    fun giftTo_whenPlayerIsNotReadyAndBloom1StackHasCards_givesCardFromBloom1() {
        ...
        every { mockMarket.getCardsFor(MarketStackID.BLOOM_1) } returns mockGameCardIds1
        
        // Act
        SUT.giftTo(mockPlayer)
        
        // Assert
        verify { mockMarket.removeCard(cardId) }
        verify { mockPlayer.addCardToCompost(cardId) }
        
        // Verify we didn't check stacks 2 and 3
        verify(exactly = 0) { mockMarket.getCardsFor(MarketStackID.BLOOM_2) }
        verify(exactly = 0) { mockMarket.getCardsFor(MarketStackID.BLOOM_3) }
    }
    ...
    fun giftTo_whenPlayerIsNotReadyAndBloom1StackIsEmptyButBloom2HasCards_givesCardFromBloom2() {
        ...
        every { mockMarket.getCardsFor(MarketStackID.BLOOM_2) } returns mockGameCardIds2
    ...
    fun giftTo_whenPlayerIsNotReadyAndOnlyBloom3StackHasCards_givesCardFromBloom3() {
        ...
        every { mockMarket.getCardsFor(MarketStackID.BLOOM_3) } returns mockGameCardIds3
    ...
    fun giftTo_whenPlayerIsNotReadyButNoBloomStacksHaveCards_doesNothing() {
        ...
        // Assert
        verify(exactly = 0) { mockMarket.removeCard(any<CardID>()) }
    ...
    fun giftTo_whenPlayerIsReady_doesNothing() {
        ...
        // Assert
        verify(exactly = 0) { mockMarket.getCardsFor(any()) }
        verify(exactly = 0) { mockMarket.removeCard(any<CardID>()) }
        verify(exactly = 0) { mockPlayer.addCardToCompost(any<CardID>()) }
    }
    ...
    fun giftTo_whenPlayerCanGetABloomCard_getsACard() {
        ...
        // Assert
        verify { mockMarket.removeCard(cardId1) }
    ...
    fun giftTo_whenMarketStackReturnsNull_skipsToNextStack() {
        ...
        every { mockMarket.getCardsFor(MarketStackID.BLOOM_1) } returns null
