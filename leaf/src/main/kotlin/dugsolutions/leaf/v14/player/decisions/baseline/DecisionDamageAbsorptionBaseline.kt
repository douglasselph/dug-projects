...
import dugsolutions.leaf.v14.player.decisions.core.DecisionDamageAbsorption.*
import dugsolutions.leaf.v14.player.domain.ExtendedHandItem
...
class DecisionDamageAbsorptionBaseline(
    ...
    override suspend operator fun invoke(): DecisionDamageAbsorption.Result {
        val amount = player.incomingDamage
        if (amount <= 0) {
            return DecisionDamageAbsorption.Result()
        }
        val handItems = player.getExtendedHandItems()

        // Check if cards/dice exist only in hand (none in compost/supply)
        val onlyCardsInHand = player.cardsInDiscard.isEmpty() && player.cardsInSupply.isEmpty()
        val onlyDiceInHand = player.diceInDiscard.isEmpty() && player.diceInSupply.isEmpty()
        ...
        val bestCombination = selectBestCombination(player, validCombinations) ?: return DecisionDamageAbsorption.Result()
