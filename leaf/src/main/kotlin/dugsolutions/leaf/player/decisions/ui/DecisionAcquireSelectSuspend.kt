...
class DecisionAcquireSelectSuspend : DecisionAcquireSelect {
    ...
    private val drawCountChannel = DecisionSuspensionChannel<DecisionAcquireSelect.BuyItem>()
    ...
    fun provide(card: GameCard) {
        ...
        require(choice != null)
        drawCountChannel.provideDecision(DecisionAcquireSelect.BuyItem.Card(choice))
    }

    fun provide(die: Die) {
        val choice = possibleDiceStash.find { it.die == die }
        require(choice != null)
        drawCountChannel.provideDecision(DecisionAcquireSelect.BuyItem.Die(choice))
    }
